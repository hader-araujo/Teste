...
- **Cliente:** pode cancelar apenas seus próprios itens (itens vinculados a suas pessoas), e somente se o status for **Na fila** (`ORDER_QUEUED`). **Não pode cancelar itens em preparo, prontos ou entregues** — evita prejuízo ao restaurante por insumo já consumido.
...
- **Sem limite de JoinRequests pendentes simultâneos por mesa** (risco aceito). Cada JoinRequest exige verificação WhatsApp via OTP (número real), o que inviabiliza spam em massa. O rate limit de OTP (`otpMaxSendsPerPhone`) + rate limit por IP são barreiras suficientes.
...
- **Carrinho:** ao adicionar item, selecionar pelo menos 1 pessoa (obrigatório). Valor divide igual entre selecionados. Cada item pode conter **observações** (`notes?: string`) — campo de texto livre para instruções especiais (ex: "bem passado", "sem cebola", "alergia a amendoim"). **Bloqueio por pagamento pendente:** pessoas com Payment `PAYMENT_PENDING` não podem ser selecionadas para novos itens — aparecem com ícone de cadeado + "pagamento pendente" no seletor de pessoas. Validação no `POST /orders`: se qualquer `personId` tem Payment `PAYMENT_PENDING`, retorna erro `ORDER_007: Pessoa com pagamento pendente — aguarde conclusão ou cancele o pagamento antes de adicionar itens`. As outras pessoas da mesa continuam pedindo normalmente.
