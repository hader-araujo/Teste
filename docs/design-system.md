...
### Por que tres interfaces separadas

- **Objetivos opostos:** o cliente quer pedir em 30 segundos; o admin quer analisar dados por 30 minutos; o cozinheiro quer ver o proximo pedido sem tocar na tela.
- **Dispositivos diferentes:** celular (cliente/garcom), tablet/TV (KDS/caixa), desktop (admin).
- **Niveis de complexidade:** o cliente vê 5 telas no maximo; o admin tem dezenas de telas com formularios e relatorios.
- **Contexto de uso:** cliente esta relaxando; staff esta sob pressao; admin esta planejando.

Um layout unico para os tres seria generico demais e ruim para todos. Cada interface deve ser a melhor possivel para seu publico.
...
| Aspecto | Cliente | Staff (KDS) | Staff (Garcom) | Admin |
|---|---|---|---|---|
| **Tema** | Light | Dark | Light | Light + sidebar dark |
| **Fundo** | Branco puro | `gray-900` | Branco puro | `gray-50` |
| **Densidade** | Baixa | Alta | Media | Media-alta |
| **Cantos** | Muito arredondados (`2xl`) | Moderados (`lg`) | Arredondados (`xl`) | Arredondados (`xl`) |
| **Sombras** | Medias (`shadow-md`) | Nenhuma (borda) | Leves (`shadow-sm`) | Leves (`shadow-sm`) |
| **Cor primaria visivel** | Muito (botoes, precos, carrinho) | Pouco (foco em semanticas) | Moderado (CTAs, tabs) | Moderado (sidebar, KPIs, CTAs) |
| **Cores semanticas** | Pouco (status do pedido) | Dominante (tudo por cor) | Muito (status das mesas) | Moderado (KPIs, badges) |
| **Font size base** | `text-base` (16px) | `text-lg` a `text-xl` (18-20px) | `text-base` (16px) | `text-sm` (14px) |
| **Touch target minimo** | 44x44px | 56x56px | 48x48px | 36x36px (mouse) |
